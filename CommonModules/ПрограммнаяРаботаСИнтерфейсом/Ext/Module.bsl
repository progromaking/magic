...
// Скопированные адреса из Outlook приводит к правильному формату, каждый адрес с новой строки, пока удаляет имена 
&НаКлиенте
Процедура ОткорректироватьАдресаЭлектроннойПочты(Адреса)
	
	ТекстПоСтрокам = СтрЗаменить(Адреса, ";", Символы.ПС);
	Результат = "";
	
	Для НомерСтрокиАдресов = 1 по СтрЧислоСтрок(ТекстПоСтрокам) Цикл
		ТекАдрес = СтрПолучитьСтроку(ТекстПоСтрокам, НомерСтрокиАдресов);
		ТекАдрес = СтрЗаменить(ТекАдрес, "<", Символы.ПС);
		ТекАдрес = СтрЗаменить(ТекАдрес, ">", Символы.ПС);
		ТекАдрес = СтрЗаменить(ТекАдрес, "'", Символы.ПС);
		Для НомерЧастиАдреса = 1 по СтрЧислоСтрок(ТекАдрес) Цикл
			ЧастьАдреса = СтрПолучитьСтроку(ТекАдрес, НомерЧастиАдреса);
			Если Найти(ЧастьАдреса, "@") Тогда
				Результат = Результат + ЧастьАдреса + ";" + Символы.ПС;
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
...
&НаСервере
Функция ПолноеИмяФормы(Форма, КраткоеИмяФормы)
	
    //Форма = ЭтаФорма;
    ПозицияТочки = СтрДлина(Форма.ИмяФормы);
    Пока Сред(Форма.ИмяФормы, ПозицияТочки, 1) <> "." Цикл ПозицияТочки = ПозицияТочки - 1; КонецЦикла; //
    Возврат Лев(Форма.ИмяФормы, ПозицияТочки) + КраткоеИмяФормы;
